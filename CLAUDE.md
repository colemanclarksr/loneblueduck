...
3. **Scan.** Universe: all U.S. stocks, no price, cap, volume, or spread floor (Coleman, 2026-09-04). Trend template first, then setup, pivot, stop, earnings, catalyst. Fill one candidate JSON per survivor with a one line reason per sub-score. Max five.
...
5. **BUY NOW requires:** trend template pass, grade B or better (score 75+, Coleman 2026-09-09; YELLOW no longer demands A+, it only cuts risk to two thirds of base), 0-2% above pivot (2-3% needs A+), stop 3-8%, volume CONFIRMED, no earnings inside 5 trading days, room to 2R, at least 1 whole share inside the risk budget, market not RED, drawdown under 8%.
...
8. **Owned positions** get exactly one status every session: HOLD, TRIM, SELL. Early failure rule applies, do not wait for the hard stop when a breakout fails. **Once green, never red** (Coleman 2026-09-09): traded 1% above entry, stop goes to the breakout day low; traded 2% above entry, stop goes to breakeven. Stops only move up. Check every session and replace the GTC stop when a level is earned.
...
10. **Halt** on 8% drawdown from equity high, 1.5% realized loss in one day, 3 consecutive losers, or when Coleman says stop. Resume only after a written review, at two thirds of the risk base until a winner confirms.
