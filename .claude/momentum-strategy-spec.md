...
**Status:** Reference spec. `trading-strategy.md` still governs live trades until Coleman resolves the conflicts listed in CLAUDE.md. Implemented as `tools/trade_check.py` and `tools/journal.py`.
...
| 4 | Pullback count preference | this is the 2nd or 3rd pullback in the current trend leg, not the 1st |
...
| 8 | Indicator confluence | at least ONE of: Bull Bear Algo bullish signal, RSI turning up from below 50, MACD histogram turning positive |
...
After a win closes, log answers to:
...
**Mindset rule:** grace when struggling, scrutiny when winning.

## 7. Trade Journal Schema
```
{
  "ticker": "", "date": "", "catalyst": "", "pullback_number": null,
  "entry_price": null, "stop_price": null, "stop_method": "flat_pct | atr",
  "target_price": null, "position_size_pct_risk": null, "num_entries_scaled": null,
  "indicator_confluence": [], "regime_ok_at_entry": true,
  "exit_price": null, "exit_reason": "target_hit | stop_hit | trend_break | discretionary",
  "result_pct": null,
  "post_win_review": {
    "size_up_opportunity": "", "matched_other_winners": "", "ev_captured_pct": null,
...
- Indicators: SMA(20), SMA(200) on SPY, ATR(14), RSI(14), MACD(12,26,9), Bull Bear Algo signal (not available through Robinhood, manual input)
- Candle pattern detection: bullish engulfing, three green soldiers, morning star, tweezer bottom
- Build order: (1) regime filter check, (2) universe scanner, (3) position sizing calculator, (4) trade journal logger, (5) post-win review prompt on any closed trade with positive result_pct
