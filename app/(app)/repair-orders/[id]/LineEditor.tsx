...
export function LineRow({ line, repairOrderId, editable, showCost }: {
  line: Line; repairOrderId: string; editable: boolean; showCost: boolean;
}) {
      ...
      <tr className="border-t border-slate-100 dark:border-slate-800">
        <td colSpan={showCost ? 6 : 5} className="p-2">
          ...
          >
            ...
            <select name="kind" defaultValue={line.kind} className={input} aria-label="Type">
              {KINDS.map((k) => <option key={k.value} value={k.value}>{k.label}</option>)}
            </select>
            <input name="name" defaultValue={line.name} className={input} required aria-label="Description" />
...
/**
 * Adds work found on the lift. The section defaults to the repair being looked
 * at; "Additional work" is the catch-all so an upsell is visibly separate from
 * what the customer originally approved.
 */
export function AddLine({ repairOrderId, section, laborRateCents }: {
  repairOrderId: string; section: string; laborRateCents: number;
}) {
...
/** Starts a new repair on a job already underway. */
export function AddSection({ repairOrderId, laborRateCents }: { repairOrderId: string; laborRateCents: number }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
      <input
        ...
        placeholder="Repair name, e.g. Front axle"
        autoFocus
        className={`${input} mb-3`}
        aria-label="Repair name"
      />
      {name.trim() ? (
        <AddLine repairOrderId={repairOrderId} section={name.trim()} laborRateCents={laborRateCents} />
      ) : (
